...
function writeSkillContent(name: string, content: string): void {
  const repoPath = run("ai-inst repo path");
...
server.tool(
  "migrate",
  "Apply pending migrations or create a new migration. Migrations are YAML rules in the rules repo (migrations/*.yml) that auto-update project config when modules are renamed, split, or extracted into skills. Called automatically during build; use this tool directly to preview, apply, or create migrations.",
  {
    project_path: z.string().describe("Absolute path to the project directory"),
